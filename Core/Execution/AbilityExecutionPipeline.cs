...
public sealed class AbilityExecutionPipeline
{
    ...
    private readonly FrameworkEffects.IEffectExecutor _effectExecutor;
    ...
    /// <param name="effectExecutor">The effect executor for applying ability effects.</param>
    public AbilityExecutionPipeline(
        AbilityRegistry abilityRegistry,
        ITargetingValidator targetingValidator,
        FrameworkEffects.IEffectExecutor effectExecutor)
    {
    ...
    public AbilityExecutionResult Execute(AbilityCommand command, EntityStore world, Entity caster)
    {
        ...
        var applyResult = ApplyEffects(command, caster, _effectExecutor);
    ...
    /// <summary>
    /// Applies the ability's effects using the provided executor.
    /// </summary>
    public EffectApplyResult ApplyEffects(AbilityCommand command, Entity caster, FrameworkEffects.IEffectExecutor executor)
    {
        if (executor == null)
        {
            return EffectApplyResult.Failure("No effect executor provided");
        }

        var abilityDef = _abilityRegistry.Get(command.AbilityId);
        foreach (var effect in abilityDef.Effects)
        {
            executor.Execute(effect, default);
        }
